function    [a f1 f2 F1 F2 M1 M2 S1 S2 P0 F0 rho z V c z0 V0 c0 F1temp A B]    =Coef(Np, muP,lambdaP, Nc, muC, lambdaC , c_p, c_c)
...
z0=min(z); %maximum negative Eigenvalue
V0=[]; % Correspond
for k=1:d_p 
    ...
    F1temp=@(x)F1temp(x)-a(k)*sum(V(:,L(k)))*exp( D(L(k),L(k))*x);
    
end

f2_0=@(t)(muP/(lambdaP+muP))^Np * (muC/(lambdaC+muC))^Nc *(1-exp(-(Np*muP+Nc*muC)*t));
F1=@(x)0;
...
F2=@(t)1-F2(t);
 ...
 c0=min(c_d(2:(Np+1)*(Nc+1))); % Minimum non-zero channel rate
